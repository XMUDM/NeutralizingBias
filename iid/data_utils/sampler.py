...
class Sampler(object):
    ...
    def __init__(self, flags_obj, lil_record, dok_record, neg_sample_rate):
        ...
        self.lil_record = lil_record  # 用两个列表来表示矩阵，一个表示第k行的元素(data)，一个表示元素在第k行的索引(rows)
        self.record = list(dok_record.keys())  # 用字典来表示矩阵，索引为位置 (m,n)
...
class PointSampler(Sampler):
    ...
    def sample(self, index):
        ...
        users = np.full(1 + self.neg_sample_rate, user, dtype=np.int64)  # user 是真实的
        items = np.full(1 + self.neg_sample_rate, pos_item, dtype=np.int64)  # 第一个item是真的，后面都是采样来的
        labels = np.zeros(1 + self.neg_sample_rate, dtype=np.float32)
        labels[0] = 1.0  # 第一个为真，后面都是假
