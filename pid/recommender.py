...
class PIDRecommender(MFRecommender):

    def __init__(self, flags_obj, workspace, dm):

        super(PIDRecommender, self).__init__(flags_obj, workspace, dm)
    ...
    def pair_inference(self, sample):
        ...
        p_score, n_score = super(PIDRecommender, self).pair_inference(sample_wrap)
        weight = weight.to(self.device)
        return p_score, n_score, weight
    
    def set_model(self):

        self.model = model.PIDMF(self.dm.n_user, self.dm.n_item, self.flags_obj.embedding_size)
